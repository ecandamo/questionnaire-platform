...
export async function POST(req: NextRequest) {
  ...
  if (parsed.errors.length > 0) {
    ...
    return NextResponse.json(
      { error: "CSV parse error", details: first?.message ?? "Unknown", row: first?.row },
      { status: 400 }
    )
  }
  ...
  return withRls(
    ...
    async (tx) => {
      const categories = await tx.select().from(questionCategory)
      ...
      for (const row of rows) {
        if (
          row.categoryName &&
          !categoryByName.has(row.categoryName) &&
      ...
      if (categoryErrors.length > 0) {
        return NextResponse.json(
          { error: "Validation failed", errors: categoryErrors },
          { status: 400 }
        )
      }
      ...
      try {
          ...
          if (row.categoryName) {
            ...
            if (!cid && createMissingCategories) {
              const [created] = await tx
                ...
                .values({ name: row.categoryName, description: null, sortOrder: 0 })
                .returning()
              cid = created!.id
        ...
        await logAudit(
          {
            ...
            entityType: "question",
            entityId: null,
            metadata: { fileName, created: { questions: rows.length } },
          },
          tx
        )
        ...
        // Manual rollback: delete inserted rows since neon-http does not
        // guarantee the tx aborts on JS throw
