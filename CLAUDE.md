...
- **Query lists with `inArray`, never `and(...eq)`:** to filter a column by a list of values use `inArray(col, ids)`. Do NOT write `and(...ids.map(id => eq(col, id)))` — that means `col = a AND col = b …`, which is **always false for 2+ ids** and silently returns zero rows (this caused the template-copy blank-questions bug; see `LRN-20260819-001`). When an infra refactor (e.g. RLS) rewrites a route, re-verify unrelated query logic in that route.
